...
meta-default-description = Automated customer messaging for small businesses. Auto-reply across WhatsApp, Instagram DMs, and email. Prepaid credits, no contracts.
...
demo-chat-business-goal = Goal
demo-chat-handoff-chip = A teammate has been notified. Concierge is holding the conversation until a human takes over.
...
features-card-step-1-eyebrow = Connect
...
wizard-launch-credits-note = Optional: top up later from Billing whenever you need credits.
...
admin-settings-conversation-h2 = Conversation Timing
admin-settings-conversation-lead = How long a customer thread stays "the same conversation," and what happens after a handoff. Leave any field blank to use the default.
admin-settings-conversation-idle-label = Idle gap
admin-settings-conversation-idle-help = After this much silence, the next message starts a fresh conversation and clears the AI's memory of the prior turns.
admin-settings-conversation-cooldown-label = Handoff cooldown
admin-settings-conversation-cooldown-help = After the AI hands off to a person, replies stay short and reassuring for this long; past it, the AI goes silent.
admin-settings-conversation-history-label = History size
admin-settings-conversation-history-help = How many recent turns the AI sees as context on each reply.
admin-settings-conversation-unit-mins = minutes
admin-settings-conversation-unit-turns = turns
admin-settings-conversation-default-prefix = Default:
...
wizard-replies-goal-url-placeholder = https://example.com/book
...
admin-persona-placeholder-goal-url = https://example.com/book
admin-persona-label-handoff = Hand off to a human if (one per line, max 5)
admin-persona-placeholder-handoff = refund or complaint &#10; the customer is upset &#10; order is over ₹5000
admin-persona-hint-handoff = Optional but recommended. The AI will stop and notify you when any of these come up.
...
admin-persona-envelope-note = Concierge wraps every prompt with a fixed safety preamble and postamble. They aren't editable; you control the middle section.
admin-prompt-preamble-label = Built-in preamble (not editable)
admin-prompt-postamble-label = Built-in safety postamble (not editable)
...
admin-rules-form-envelope-note = Concierge wraps the combined prompt in a fixed safety preamble + postamble before it reaches the model. View the active envelope on the persona page.
