...
// (name, sri) for every file in public/css/, in dependency order.
// Generated by build.rs from the files on disk — no CSS bytes ride
// along in the worker binary.
include!(concat!(env!("OUT_DIR"), "/css_sri.rs"));

fn css_link_tags() -> String {
    CSS_MODULES
        .iter()
        .map(|(name, sri)| {
            format!(
                r#"<link rel="stylesheet" href="/css/{name}" integrity="{sri}" nonce="__CSP_NONCE__">"#
            )
        })
        .collect::<Vec<_>>()
        .join("\n")
